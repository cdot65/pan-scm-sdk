"""Authentication Rule configuration service for Strata Cloud Manager SDK.

Provides service class for managing authentication rule objects via the SCM API.
"""

# scm/config/security/authentication_rule.py
...
class AuthenticationRule(BaseObject):
    """Manages Authentication Rule objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/identity/v1/authentication-rules"
    ...
    def create(
        ...
        if not isinstance(rulebase, AuthenticationRuleRulebase):
            try:
                rulebase = AuthenticationRuleRulebase(rulebase.lower())
            except ValueError:
                raise InvalidObjectError(
        ...
        profile = AuthenticationRuleCreateModel(**data)
        ...
        return AuthenticationRuleResponseModel(**response)
    ...
    def get(
        ...
        if not isinstance(rulebase, AuthenticationRuleRulebase):
            try:
                AuthenticationRuleRulebase(rulebase.lower())
            except ValueError:
                raise InvalidObjectError(
                    message="rulebase must be either 'pre' or 'post'",
                    error_code="E003",
    ...
    def update(
        self,
        rule: AuthenticationRuleUpdateModel,
        rulebase: str = "pre",
    ) -> AuthenticationRuleResponseModel:
        """Update an existing authentication rule object.

        Args:
            rule: AuthenticationRuleUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        rules: List[AuthenticationRuleResponseModel],
        filters: Dict[str, Any],
    ) -> List[AuthenticationRuleResponseModel]:
        """Apply client-side filtering to the list of authentication rules.

        Args:
            rules: List of AuthenticationRuleResponseModel objects
    ...
    def list(
        ...
        **filters,
        ...
        """List authentication rule objects with optional filtering.
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [AuthenticationRuleResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single authentication rule by name.

        Args:
            name (str): The name of the authentication rule to fetch.
    ...
    def delete(
        ...
        """Delete an authentication rule object.
    ...
    def move(
        ...
        """Move an authentication rule to a new position within the rulebase.
        ...
        move_config = AuthenticationRuleMoveModel(**data)
