"""IPsec Tunnel configuration service for Strata Cloud Manager SDK.

Provides service class for managing IPsec tunnel objects via the SCM API.
"""

# scm/config/network/ipsec_tunnel.py
...
class IPsecTunnel(BaseObject):
    """Manages IPsec Tunnel objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/ipsec-tunnels"
    ...
    def create(
        ...
        """Create a new IPsec tunnel object.

        Args:
            data: Dictionary containing the IPsec tunnel configuration
        ...
        return IPsecTunnelResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> IPsecTunnelResponseModel:
        """Get an IPsec tunnel object by ID.

        Args:
            object_id: The ID of the IPsec tunnel to retrieve
    ...
    def update(
        self,
        tunnel: IPsecTunnelUpdateModel,
    ) -> IPsecTunnelResponseModel:
        """Update an existing IPsec tunnel object.

        Args:
            tunnel: IPsecTunnelUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        tunnels: List[IPsecTunnelResponseModel],
        filters: Dict[str, Any],
    ) -> List[IPsecTunnelResponseModel]:
        """Apply client-side filtering to the list of IPsec tunnels.

        Args:
            tunnels: List of IPsecTunnelResponseModel objects
        ...
        # Filter by ipsec_crypto_profile name
        if "ipsec_crypto_profile" in filters:
            if not isinstance(filters["ipsec_crypto_profile"], list):
                raise InvalidObjectError(
                    message="'ipsec_crypto_profile' filter must be a list",
                    error_code="E003",
                    http_status_code=400,
                    details={"errorType": "Invalid Object"},
                )
            profiles = filters["ipsec_crypto_profile"]
            filter_criteria = [
                tunnel
                for tunnel in filter_criteria
                if tunnel.auto_key.ipsec_crypto_profile in profiles
            ]
    ...
    def list(
        ...
        **filters,
        ...
        """List IPsec tunnel objects with optional filtering.
        ...
        Args:
            ...
            **filters: Additional filters including:
                - ipsec_crypto_profile: List[str] - Filter by IPsec crypto profile names
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [IPsecTunnelResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single IPsec tunnel by name.

        Args:
            name: The name of the IPsec tunnel to fetch
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"IPsec tunnel '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching IPsec tunnel found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple IPsec tunnels found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return IPsecTunnelResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
