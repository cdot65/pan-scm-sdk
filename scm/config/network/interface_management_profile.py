"""Interface Management Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing interface management profile objects via the SCM API.
"""

# scm/config/network/interface_management_profile.py
...
class InterfaceManagementProfile(BaseObject):
    """Manages Interface Management Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/interface-management-profiles"
    ...
    def create(
        ...
        """Create a new interface management profile object.

        Args:
            data: Dictionary containing the interface management profile configuration
        ...
        profile = InterfaceManagementProfileCreateModel(**data)
        ...
        return InterfaceManagementProfileResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> InterfaceManagementProfileResponseModel:
        """Get an interface management profile object by ID.

        Args:
            object_id: The ID of the interface management profile to retrieve
    ...
    def update(
        self,
        profile: InterfaceManagementProfileUpdateModel,
    ) -> InterfaceManagementProfileResponseModel:
        """Update an existing interface management profile object.

        Args:
            profile: InterfaceManagementProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[InterfaceManagementProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[InterfaceManagementProfileResponseModel]:
        """Apply client-side filtering to the list of interface management profiles.

        Args:
            profiles: List of InterfaceManagementProfileResponseModel objects
        ...
        for field_name in boolean_fields:
            if field_name in filters:
                if not isinstance(filters[field_name], bool):
                    raise InvalidObjectError(
                        message=f"'{field_name}' filter must be a boolean",
                        error_code="E003",
                        http_status_code=400,
    ...
    def list(
        ...
        **filters,
        ...
        """List interface management profile objects with optional filtering.
        ...
        Args:
            ...
            **filters: Additional filters including:
                - http: bool - Filter by HTTP enabled status
                - https: bool - Filter by HTTPS enabled status
                - telnet: bool - Filter by Telnet enabled status
                - ssh: bool - Filter by SSH enabled status
                - ping: bool - Filter by ping enabled status
                - http_ocsp: bool - Filter by HTTP OCSP enabled status
                - response_pages: bool - Filter by response pages enabled status
                - userid_service: bool - Filter by User-ID service enabled status
                - userid_syslog_listener_ssl: bool - Filter by User-ID syslog listener SSL status
                - userid_syslog_listener_udp: bool - Filter by User-ID syslog listener UDP status
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [InterfaceManagementProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single interface management profile by name.

        Args:
            name: The name of the interface management profile to fetch
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"Interface management profile '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching interface management profile found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple interface management profiles found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return InterfaceManagementProfileResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete an interface management profile object.
