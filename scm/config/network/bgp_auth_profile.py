"""BGP Authentication Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing BGP authentication profile objects via the SCM API.
"""

# scm/config/network/bgp_auth_profile.py
...
class BgpAuthProfile(BaseObject):
    """Manages BGP Authentication Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/bgp-auth-profiles"
    ...
    def get(
        ...
        """Get a BGP authentication profile object by ID.

        Args:
            object_id: The ID of the BGP authentication profile to retrieve
    ...
    def update(
        self,
        profile: BgpAuthProfileUpdateModel,
    ) -> BgpAuthProfileResponseModel:
        """Update an existing BGP authentication profile object.

        Args:
            profile: BgpAuthProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[BgpAuthProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[BgpAuthProfileResponseModel]:
        """Apply client-side filtering to the list of BGP authentication profiles.

        Args:
            profiles: List of BgpAuthProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[BgpAuthProfileResponseModel]: Filtered list of BGP authentication profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List BGP authentication profile objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [BgpAuthProfileResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [BgpAuthProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single BGP authentication profile by name.

        Args:
            name: The name of the BGP authentication profile to fetch
        ...
        if isinstance(response, list):
            ...
            return BgpAuthProfileResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"BGP authentication profile '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching BGP authentication profile found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple BGP authentication profiles found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return BgpAuthProfileResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
