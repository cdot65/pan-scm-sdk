"""Route Access List configuration service for Strata Cloud Manager SDK.

Provides service class for managing route access list objects via the SCM API.
"""

# scm/config/network/route_access_list.py
...
class RouteAccessList(BaseObject):
    """Manages Route Access List objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/route-access-lists"
    ...
    def create(
        ...
        """Create a new route access list object.

        Args:
            data: Dictionary containing the route access list configuration
        ...
        resource = RouteAccessListCreateModel(**data)
        ...
        return RouteAccessListResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> RouteAccessListResponseModel:
        """Get a route access list object by ID.

        Args:
            object_id: The ID of the route access list to retrieve
    ...
    def update(
        self,
        resource: RouteAccessListUpdateModel,
    ) -> RouteAccessListResponseModel:
        """Update an existing route access list object.

        Args:
            resource: RouteAccessListUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        resources: List[RouteAccessListResponseModel],
        filters: Dict[str, Any],
    ) -> List[RouteAccessListResponseModel]:
        """Apply client-side filtering to the list of route access lists.

        Args:
            resources: List of RouteAccessListResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[RouteAccessListResponseModel]: Filtered list of route access lists

        """
        filter_criteria = resources
    ...
    def list(
        ...
        **filters,
        ...
        """List route access list objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [RouteAccessListResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [RouteAccessListResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single route access list by name.

        Args:
            name: The name of the route access list to fetch
        ...
        if isinstance(response, list):
            ...
            return RouteAccessListResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"Route access list '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching route access list found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple route access lists found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return RouteAccessListResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a route access list object.
