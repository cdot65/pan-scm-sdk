"""Aggregate Interface configuration service for Strata Cloud Manager SDK.

Provides service class for managing aggregate interface objects via the SCM API.
"""
...
class AggregateInterface(BaseObject):
    """Manages Aggregate Interface objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/aggregate-interfaces"
    ...
    def list(
        ...
        **filters,
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [AggregateInterfaceResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        if "id" in response:
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"Aggregate interface '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching aggregate interface found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(f"Multiple aggregate interfaces found for '{name}'. Using the first one.")
            return AggregateInterfaceResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
