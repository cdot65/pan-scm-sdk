"""QoS Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing QoS profile objects via the SCM API.
"""

# scm/config/network/qos_profile.py
...
class QosProfile(BaseObject):
    """Manages QoS Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/qos-profiles"
    ...
    def create(
        ...
        """Create a new QoS profile object.

        Args:
            data: Dictionary containing the QoS profile configuration
        ...
        profile = QosProfileCreateModel(**data)
        ...
        return QosProfileResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> QosProfileResponseModel:
        """Get a QoS profile object by ID.

        Args:
            object_id: The ID of the QoS profile to retrieve
    ...
    def update(
        self,
        profile: QosProfileUpdateModel,
    ) -> QosProfileResponseModel:
        """Update an existing QoS profile object.

        Args:
            profile: QosProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[QosProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[QosProfileResponseModel]:
        """Apply client-side filtering to the list of QoS profiles.

        Args:
            profiles: List of QosProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[QosProfileResponseModel]: Filtered list of QoS profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List QoS profile objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [QosProfileResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [QosProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single QoS profile by name.

        Args:
            name: The name of the QoS profile to fetch
        ...
        if isinstance(response, list):
            ...
            return QosProfileResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"QoS profile '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching QoS profile found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple QoS profiles found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return QosProfileResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
