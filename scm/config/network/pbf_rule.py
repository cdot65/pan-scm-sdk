"""PBF Rule configuration service for Strata Cloud Manager SDK.

Provides service class for managing Policy-Based Forwarding rule objects via the SCM API.
"""

# scm/config/network/pbf_rule.py
...
class PbfRule(BaseObject):
    """Manages PBF Rule objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/pbf-rules"
    ...
    def create(
        ...
        """Create a new PBF rule object.

        Args:
            data: Dictionary containing the PBF rule configuration
        ...
        return PbfRuleResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> PbfRuleResponseModel:
        """Get a PBF rule object by ID.

        Args:
            object_id: The ID of the PBF rule to retrieve
    ...
    def update(
        self,
        rule: PbfRuleUpdateModel,
    ) -> PbfRuleResponseModel:
        """Update an existing PBF rule object.

        Args:
            rule: PbfRuleUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        rules: List[PbfRuleResponseModel],
        filters: Dict[str, Any],
    ) -> List[PbfRuleResponseModel]:
        """Apply client-side filtering to the list of PBF rules.

        Args:
            rules: List of PbfRuleResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[PbfRuleResponseModel]: Filtered list of PBF rules
    ...
    def list(
        ...
        **filters,
        ...
        """List PBF rule objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [PbfRuleResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [PbfRuleResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single PBF rule by name.

        Args:
            name: The name of the PBF rule to fetch
        ...
        if isinstance(response, list):
            ...
            return PbfRuleResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"PBF rule '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching PBF rule found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(f"Multiple PBF rules found for '{name}'. Using the first one.")
            # Return the first item in the data array
            return PbfRuleResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
