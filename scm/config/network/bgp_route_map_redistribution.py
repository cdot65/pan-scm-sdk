"""BGP Route Map Redistribution configuration service for Strata Cloud Manager SDK.

Provides service class for managing BGP route map redistribution objects via the SCM API.
"""

# scm/config/network/bgp_route_map_redistribution.py
...
class BgpRouteMapRedistribution(BaseObject):
    """Manages BGP Route Map Redistribution objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/bgp-route-map-redistributions"
    ...
    def create(
        ...
        """Create a new BGP route map redistribution object.

        Args:
            data: Dictionary containing the BGP route map redistribution configuration
        ...
        profile = BgpRouteMapRedistributionCreateModel(**data)
        ...
        return BgpRouteMapRedistributionResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> BgpRouteMapRedistributionResponseModel:
        """Get a BGP route map redistribution object by ID.

        Args:
            object_id: The ID of the BGP route map redistribution to retrieve
    ...
    def update(
        self,
        profile: BgpRouteMapRedistributionUpdateModel,
    ) -> BgpRouteMapRedistributionResponseModel:
        """Update an existing BGP route map redistribution object.

        Args:
            profile: BgpRouteMapRedistributionUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[BgpRouteMapRedistributionResponseModel],
        filters: Dict[str, Any],
    ) -> List[BgpRouteMapRedistributionResponseModel]:
        """Apply client-side filtering to the list of BGP route map redistributions.

        Args:
            profiles: List of BgpRouteMapRedistributionResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[BgpRouteMapRedistributionResponseModel]: Filtered list of BGP route map redistributions
    ...
    def list(
        ...
        **filters,
        ...
        """List BGP route map redistribution objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [BgpRouteMapRedistributionResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [BgpRouteMapRedistributionResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single BGP route map redistribution by name.

        Args:
            name: The name of the BGP route map redistribution to fetch
        ...
        if isinstance(response, list):
            ...
            return BgpRouteMapRedistributionResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"BGP route map redistribution '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching BGP route map redistribution found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple BGP route map redistributions found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return BgpRouteMapRedistributionResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a BGP route map redistribution object.
