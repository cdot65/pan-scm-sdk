"""BGP Filtering Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing BGP filtering profile objects via the SCM API.
"""

# scm/config/network/bgp_filtering_profile.py
...
class BgpFilteringProfile(BaseObject):
    """Manages BGP Filtering Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/bgp-filtering-profiles"
    ...
    def create(
        ...
        """Create a new BGP filtering profile object.

        Args:
            data: Dictionary containing the BGP filtering profile configuration
        ...
        profile = BgpFilteringProfileCreateModel(**data)
        ...
        return BgpFilteringProfileResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> BgpFilteringProfileResponseModel:
        """Get a BGP filtering profile object by ID.

        Args:
            object_id: The ID of the BGP filtering profile to retrieve
    ...
    def update(
        self,
        profile: BgpFilteringProfileUpdateModel,
    ) -> BgpFilteringProfileResponseModel:
        """Update an existing BGP filtering profile object.

        Args:
            profile: BgpFilteringProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[BgpFilteringProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[BgpFilteringProfileResponseModel]:
        """Apply client-side filtering to the list of BGP filtering profiles.

        Args:
            profiles: List of BgpFilteringProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[BgpFilteringProfileResponseModel]: Filtered list of BGP filtering profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List BGP filtering profile objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [BgpFilteringProfileResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [BgpFilteringProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single BGP filtering profile by name.

        Args:
            name: The name of the BGP filtering profile to fetch
        ...
        if isinstance(response, list):
            ...
            return BgpFilteringProfileResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"BGP filtering profile '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching BGP filtering profile found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple BGP filtering profiles found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return BgpFilteringProfileResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a BGP filtering profile object.
