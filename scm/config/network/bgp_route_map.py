"""BGP Route Map configuration service for Strata Cloud Manager SDK.

Provides service class for managing BGP route map objects via the SCM API.
"""

# scm/config/network/bgp_route_map.py
...
class BgpRouteMap(BaseObject):
    """Manages BGP Route Map objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/bgp-route-maps"
    ...
    def create(
        ...
        """Create a new BGP route map object.

        Args:
            data: Dictionary containing the BGP route map configuration
        ...
        profile = BgpRouteMapCreateModel(**data)
        ...
        return BgpRouteMapResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> BgpRouteMapResponseModel:
        """Get a BGP route map object by ID.

        Args:
            object_id: The ID of the BGP route map to retrieve
    ...
    def update(
        self,
        profile: BgpRouteMapUpdateModel,
    ) -> BgpRouteMapResponseModel:
        """Update an existing BGP route map object.

        Args:
            profile: BgpRouteMapUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[BgpRouteMapResponseModel],
        filters: Dict[str, Any],
    ) -> List[BgpRouteMapResponseModel]:
        """Apply client-side filtering to the list of BGP route maps.

        Args:
            profiles: List of BgpRouteMapResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[BgpRouteMapResponseModel]: Filtered list of BGP route maps
    ...
    def list(
        ...
        **filters,
        ...
        """List BGP route map objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [BgpRouteMapResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [BgpRouteMapResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single BGP route map by name.

        Args:
            name: The name of the BGP route map to fetch
        ...
        if isinstance(response, list):
            ...
            return BgpRouteMapResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"BGP route map '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching BGP route map found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple BGP route maps found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return BgpRouteMapResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
