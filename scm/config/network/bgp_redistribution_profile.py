"""BGP Redistribution Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing BGP redistribution profile objects via the SCM API.
"""

# scm/config/network/bgp_redistribution_profile.py
...
class BgpRedistributionProfile(BaseObject):
    """Manages BGP Redistribution Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/bgp-redistribution-profiles"
    ...
    def create(
        ...
        """Create a new BGP redistribution profile object.

        Args:
            data: Dictionary containing the BGP redistribution profile configuration
        ...
        profile = BgpRedistributionProfileCreateModel(**data)
        ...
        return BgpRedistributionProfileResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> BgpRedistributionProfileResponseModel:
        """Get a BGP redistribution profile object by ID.

        Args:
            object_id: The ID of the BGP redistribution profile to retrieve
    ...
    def update(
        self,
        profile: BgpRedistributionProfileUpdateModel,
    ) -> BgpRedistributionProfileResponseModel:
        """Update an existing BGP redistribution profile object.

        Args:
            profile: BgpRedistributionProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[BgpRedistributionProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[BgpRedistributionProfileResponseModel]:
        """Apply client-side filtering to the list of BGP redistribution profiles.

        Args:
            profiles: List of BgpRedistributionProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[BgpRedistributionProfileResponseModel]: Filtered list of BGP redistribution profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List BGP redistribution profile objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [BgpRedistributionProfileResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [BgpRedistributionProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single BGP redistribution profile by name.

        Args:
            name: The name of the BGP redistribution profile to fetch
        ...
        if isinstance(response, list):
            ...
            return BgpRedistributionProfileResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"BGP redistribution profile '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching BGP redistribution profile found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple BGP redistribution profiles found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return BgpRedistributionProfileResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a BGP redistribution profile object.
