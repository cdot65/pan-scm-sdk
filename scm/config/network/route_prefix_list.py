"""Route Prefix List configuration service for Strata Cloud Manager SDK.

Provides service class for managing route prefix list objects via the SCM API.
"""

# scm/config/network/route_prefix_list.py
...
class RoutePrefixList(BaseObject):
    """Manages Route Prefix List objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/route-prefix-lists"
    ...
    def create(
        ...
        """Create a new route prefix list object.

        Args:
            data: Dictionary containing the route prefix list configuration
        ...
        resource = RoutePrefixListCreateModel(**data)
        ...
        return RoutePrefixListResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> RoutePrefixListResponseModel:
        """Get a route prefix list object by ID.

        Args:
            object_id: The ID of the route prefix list to retrieve
    ...
    def update(
        self,
        resource: RoutePrefixListUpdateModel,
    ) -> RoutePrefixListResponseModel:
        """Update an existing route prefix list object.

        Args:
            resource: RoutePrefixListUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        resources: List[RoutePrefixListResponseModel],
        filters: Dict[str, Any],
    ) -> List[RoutePrefixListResponseModel]:
        """Apply client-side filtering to the list of route prefix lists.

        Args:
            resources: List of RoutePrefixListResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[RoutePrefixListResponseModel]: Filtered list of route prefix lists
    ...
    def list(
        ...
        **filters,
        ...
        """List route prefix list objects with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [RoutePrefixListResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [RoutePrefixListResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single route prefix list by name.

        Args:
            name: The name of the route prefix list to fetch
        ...
        if isinstance(response, list):
            ...
            return RoutePrefixListResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"Route prefix list '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching route prefix list found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple route prefix lists found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return RoutePrefixListResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a route prefix list object.
