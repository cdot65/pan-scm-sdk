"""Tunnel Interface configuration service for Strata Cloud Manager SDK.

Provides service class for managing tunnel interface objects via the SCM API.
"""
...
class TunnelInterface(BaseObject):
    """Manages Tunnel Interface objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/network/v1/tunnel-interfaces"
    ...
    def create(
        ...
        """Create a new tunnel interface object.

        Args:
            data: Dictionary containing the tunnel interface configuration

        Returns:
            TunnelInterfaceResponseModel

        """
        tunnel = TunnelInterfaceCreateModel(**data)
        ...
        return TunnelInterfaceResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> TunnelInterfaceResponseModel:
        """Get a tunnel interface object by ID.

        Args:
            object_id: The ID of the tunnel interface to retrieve
    ...
    def update(
        self,
        tunnel: TunnelInterfaceUpdateModel,
    ) -> TunnelInterfaceResponseModel:
        """Update an existing tunnel interface object.

        Args:
            tunnel: TunnelInterfaceUpdateModel instance containing the update data
    ...
    def delete(
        ...
        """Delete a tunnel interface object.
    ...
    @staticmethod
    def _apply_filters(
        interfaces: List[TunnelInterfaceResponseModel],
        filters: Dict[str, Any],
    ) -> List[TunnelInterfaceResponseModel]:
        """Apply client-side filtering to the list of tunnel interfaces.

        Args:
            interfaces: List of TunnelInterfaceResponseModel objects
    ...
    def list(
        ...
        **filters,
        ...
        """List tunnel interface objects with optional filtering.
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [TunnelInterfaceResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single tunnel interface by name.

        Args:
            name: The name of the tunnel interface to fetch
        ...
        if "id" in response:
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"Tunnel interface '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching tunnel interface found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple tunnel interfaces found for '{name}'. Using the first one."
                )
            return TunnelInterfaceResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
