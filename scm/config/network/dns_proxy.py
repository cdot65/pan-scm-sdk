"""DNS Proxy configuration service for Strata Cloud Manager SDK.

Provides service class for managing DNS proxy objects via the SCM API.
"""

# scm/config/network/dns_proxy.py
...
class DnsProxy(BaseObject):
    """Manages DNS Proxy objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    def create(
        ...
        """Create a new DNS proxy configuration.

        Args:
            data: Dictionary containing the DNS proxy configuration
        ...
        return DnsProxyResponseModel(**response)

    def get(
        self,
        object_id: str,
    ) -> DnsProxyResponseModel:
        """Get a DNS proxy configuration by ID.

        Args:
            object_id: The ID of the DNS proxy to retrieve
    ...
    def update(
        self,
        proxy: DnsProxyUpdateModel,
    ) -> DnsProxyResponseModel:
        """Update an existing DNS proxy configuration.

        Args:
            proxy: DnsProxyUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        proxies: List[DnsProxyResponseModel],
        filters: Dict[str, Any],
    ) -> List[DnsProxyResponseModel]:
        """Apply client-side filtering to the list of DNS proxies.

        Args:
            proxies: List of DnsProxyResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[DnsProxyResponseModel]: Filtered list of DNS proxies

        """
        filter_criteria = proxies
    ...
    def list(
        ...
        **filters,
        ...
        """List DNS proxy configurations with optional filtering.
        ...
        while True:
            ...
            if isinstance(response, list):
                ...
                return [DnsProxyResponseModel(**item) for item in data]
            ...
            data = response["data"]
            object_instances = [DnsProxyResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single DNS proxy configuration by name.

        Args:
            name: The name of the DNS proxy to fetch
        ...
        if isinstance(response, list):
            ...
            return DnsProxyResponseModel(**response[0])
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if not response["data"]:
                raise InvalidObjectError(
                    message=f"DNS proxy '{name}' not found",
                    error_code="E002",
                    http_status_code=404,
                    details={"error": "No matching DNS proxy found"},
                )
            ...
            if len(response["data"]) > 1:
                self.logger.warning(
                    f"Multiple DNS proxies found for '{name}'. Using the first one."
                )
            # Return the first item in the data array
            return DnsProxyResponseModel(**response["data"][0])
        else:
            raise InvalidObjectError(
    ...
    def delete(
        ...
        """Delete a DNS proxy configuration.
