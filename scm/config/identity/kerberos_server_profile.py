"""Kerberos Server Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing Kerberos server profile objects via the SCM API.
"""

# scm/config/identity/kerberos_server_profile.py
...
class KerberosServerProfile(BaseObject):
    """Manages Kerberos Server Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    ENDPOINT = "/config/identity/v1/kerberos-server-profiles"
    ...
    def create(
        ...
        return KerberosServerProfileResponseModel(**response)
    ...
    def update(
        self,
        profile: KerberosServerProfileUpdateModel,
    ) -> KerberosServerProfileResponseModel:
        """Update an existing Kerberos server profile object.

        Args:
            profile: KerberosServerProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[KerberosServerProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[KerberosServerProfileResponseModel]:
        """Apply client-side filtering to the list of Kerberos server profiles.

        Args:
            profiles: List of KerberosServerProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[KerberosServerProfileResponseModel]: Filtered list of profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List Kerberos server profile objects with optional filtering.
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [KerberosServerProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single Kerberos server profile by name.

        Args:
            name (str): The name of the Kerberos server profile to fetch.
    ...
    def delete(
        ...
        """Delete a Kerberos server profile object.
