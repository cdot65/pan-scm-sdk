"""LDAP Server Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing LDAP server profile objects via the SCM API.
"""

# scm/config/identity/ldap_server_profile.py
...
class LdapServerProfile(BaseObject):
    """Manages LDAP Server Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    def create(
        ...
        return LdapServerProfileResponseModel(**response)
    ...
    def update(
        self,
        profile: LdapServerProfileUpdateModel,
    ) -> LdapServerProfileResponseModel:
        """Update an existing LDAP server profile object.

        Args:
            profile: LdapServerProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[LdapServerProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[LdapServerProfileResponseModel]:
        """Apply client-side filtering to the list of LDAP server profiles.

        Args:
            profiles: List of LdapServerProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[LdapServerProfileResponseModel]: Filtered list of profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List LDAP server profile objects with optional filtering.
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [LdapServerProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single LDAP server profile by name.

        Args:
            name (str): The name of the LDAP server profile to fetch.
    ...
    def delete(
        ...
        """Delete an LDAP server profile object.
