"""SAML Server Profile configuration service for Strata Cloud Manager SDK.

Provides service class for managing SAML server profile objects via the SCM API.
"""

# scm/config/identity/saml_server_profile.py
...
class SamlServerProfile(BaseObject):
    """Manages SAML Server Profile objects in Palo Alto Networks' Strata Cloud Manager.
    ...
    def create(
        ...
        return SamlServerProfileResponseModel(**response)
    ...
    def update(
        self,
        profile: SamlServerProfileUpdateModel,
    ) -> SamlServerProfileResponseModel:
        """Update an existing SAML server profile object.

        Args:
            profile: SamlServerProfileUpdateModel instance containing the update data
    ...
    @staticmethod
    def _apply_filters(
        profiles: List[SamlServerProfileResponseModel],
        filters: Dict[str, Any],
    ) -> List[SamlServerProfileResponseModel]:
        """Apply client-side filtering to the list of SAML server profiles.

        Args:
            profiles: List of SamlServerProfileResponseModel objects
            filters: Dictionary of filter criteria

        Returns:
            List[SamlServerProfileResponseModel]: Filtered list of profiles
    ...
    def list(
        ...
        **filters,
        ...
        """List SAML server profile objects with optional filtering.
        ...
        while True:
            ...
            data = response["data"]
            object_instances = [SamlServerProfileResponseModel(**item) for item in data]
    ...
    def fetch(
        ...
        """Fetch a single SAML server profile by name.

        Args:
            name (str): The name of the SAML server profile to fetch.
    ...
    def delete(
        ...
        """Delete a SAML server profile object.
