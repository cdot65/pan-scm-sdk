...
class DeviceGetResponseModel(BaseModel):
    ...
    connected_since: Optional[str] = Field(alias="connectedSince", default=None) # Consider datetime conversion?

    # Placeholder for nested models - will be filled in later
    installed_licenses: Optional[List[InstalledLicenseModel]] = Field(alias="installedLicenses", default=None)
    available_licenses: Optional[List[AvailableLicenseModel]] = Field(alias="available_licensess", default=None) # Check API for correct key
