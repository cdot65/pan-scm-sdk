...
from scm.config.identity import LdapServerProfile
from scm.exceptions import InvalidObjectError, MissingQueryParameterError
from scm.models.identity.ldap_server_profiles import LdapServerProfileResponseModel
...
class TestLdapServerProfileBase:
    """Base class for LDAP Server Profile tests."""
    ...
    def setup_method(self, mock_scm):
        ...
        self.client = LdapServerProfile(self.mock_scm, max_limit=5000)


class TestLdapServerProfileMaxLimit(TestLdapServerProfileBase):
    """Tests for LDAP Server Profile max_limit functionality."""

    def test_default_max_limit(self):
        """Test that default max_limit is set correctly."""
        client = LdapServerProfile(self.mock_scm)
        assert client.max_limit == LdapServerProfile.DEFAULT_MAX_LIMIT
        assert client.max_limit == 2500

    def test_custom_max_limit(self):
        """Test setting a custom max_limit."""
        client = LdapServerProfile(self.mock_scm, max_limit=1000)
    ...
    def test_invalid_max_limit_type(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            LdapServerProfile(self.mock_scm, max_limit="invalid")
    ...
    def test_max_limit_too_low(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            LdapServerProfile(self.mock_scm, max_limit=0)
    ...
    def test_max_limit_too_high(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            LdapServerProfile(self.mock_scm, max_limit=6000)
        assert "max_limit exceeds maximum allowed value" in str(exc_info.value)


class TestLdapServerProfileList(TestLdapServerProfileBase):
    """Tests for listing LDAP Server Profile objects."""

    def test_list_valid(self):
        """Test listing all objects successfully."""
        mock_response = {
            "data": [
                LdapServerProfileResponseFactory(name="profile1", folder="Texas").model_dump(),
                LdapServerProfileResponseFactory(name="profile2", folder="Texas").model_dump(),
            ],
            "offset": 0,
            "total": 2,
            "limit": 200,
        ...
        self.mock_scm.get.assert_called_once_with(
            "/config/identity/v1/ldap-server-profiles",
            params={"limit": 5000, "folder": "Texas", "offset": 0},
        )
        assert len(existing_objects) == 2
        assert isinstance(existing_objects[0], LdapServerProfileResponseModel)
    ...
    def test_list_exact_match(self):
        ...
        mock_response = {
            "data": [
                LdapServerProfileResponseFactory(name="p_texas", folder="Texas").model_dump(),
                LdapServerProfileResponseFactory(name="p_all", folder="All").model_dump(),
            ]
        }
        self.mock_scm.get.return_value = mock_response
    ...
    def test_list_exclude_folders(self):
        ...
        mock_response = {
            "data": [
                LdapServerProfileResponseFactory(name="p1", folder="Texas").model_dump(),
                LdapServerProfileResponseFactory(name="p2", folder="All").model_dump(),
            ]
        }
        self.mock_scm.get.return_value = mock_response
    ...
    def test_list_exclude_snippets(self):
        ...
        mock_response = {
            "data": [
                LdapServerProfileResponseFactory(name="p1", folder="Texas").model_dump(),
                LdapServerProfileResponseFactory.with_snippet(
    ...
    def test_list_exclude_devices(self):
        ...
        mock_response = {
            "data": [
                LdapServerProfileResponseFactory(name="p1", folder="Texas").model_dump(),
                LdapServerProfileResponseFactory.with_device(
    ...
    def test_list_pagination_multiple_pages(self):
        ...
        client = LdapServerProfile(self.mock_scm, max_limit=2500)
        first_page = [
            LdapServerProfileResponseFactory(name=f"p1-{i}", folder="Texas").model_dump()
            for i in range(2500)
        ]
        second_page = [
            LdapServerProfileResponseFactory(name=f"p2-{i}", folder="Texas").model_dump()
            for i in range(2500)
        ]
...
class TestLdapServerProfileCreate(TestLdapServerProfileBase):
    """Tests for creating LDAP Server Profile objects."""

    def test_create_valid_object(self):
        """Test creating an object with valid data."""
        test_object = LdapServerProfileCreateApiFactory.build()
        mock_response = LdapServerProfileResponseFactory.from_request(test_object)
        ...
        self.mock_scm.post.assert_called_once_with(
            "/config/identity/v1/ldap-server-profiles", json=test_object.model_dump()
        )
        assert isinstance(created_object, LdapServerProfileResponseModel)
...
class TestLdapServerProfileGet(TestLdapServerProfileBase):
    """Tests for retrieving LDAP Server Profile objects."""

    def test_get_valid_object(self):
        """Test retrieving a specific object."""
        mock_response = LdapServerProfileResponseFactory.build()
        self.mock_scm.get.return_value = mock_response.model_dump()
        retrieved_object = self.client.get(str(mock_response.id))
        self.mock_scm.get.assert_called_once_with(
            f"/config/identity/v1/ldap-server-profiles/{mock_response.id}"
        )
        assert isinstance(retrieved_object, LdapServerProfileResponseModel)
...
class TestLdapServerProfileUpdate(TestLdapServerProfileBase):
    """Tests for updating LDAP Server Profile objects."""

    def test_update_valid_object(self):
        """Test updating an object with valid data."""
        update_data = LdapServerProfileUpdateApiFactory(
            id="123e4567-e89b-12d3-a456-426655440000", name="updated-profile"
        )
        mock_response = LdapServerProfileResponseFactory.from_request(update_data)
        self.mock_scm.put.return_value = mock_response.model_dump()
        updated_object = self.client.update(update_data)
        self.mock_scm.put.assert_called_once()
        assert isinstance(updated_object, LdapServerProfileResponseModel)
...
class TestLdapServerProfileDelete(TestLdapServerProfileBase):
    """Tests for deleting LDAP Server Profile objects."""
    ...
    def test_delete_success(self):
        ...
        self.mock_scm.delete.assert_called_once_with(
            f"/config/identity/v1/ldap-server-profiles/{object_id}"
        )
...
class TestLdapServerProfileFetch(TestLdapServerProfileBase):
    """Tests for fetching LDAP Server Profile objects by name."""

    def test_fetch_valid_object(self):
        """Test retrieving an object by its name using the fetch method."""
        mock_response_model = LdapServerProfileResponseFactory.build()
        ...
        assert isinstance(fetched_object, LdapServerProfileResponseModel)
