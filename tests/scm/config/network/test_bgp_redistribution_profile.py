"""Unit tests for the BgpRedistributionProfile class."""
...
@pytest.fixture
def sample_bgp_redist_profile_dict():
    """Return a sample BGP redistribution profile dictionary."""
    return {
        "id": str(uuid.uuid4()),
        "name": "test-bgp-redist",
        "folder": "Test Folder",
        "ipv4": {
            "unicast": {
                "static": {"enable": True, "metric": 100, "route_map": "static-map"},
                "ospf": {"enable": True, "metric": 200},
                "connected": {"enable": False},
            },
        },
    }
...
@pytest.fixture
def sample_bgp_redist_profile_response(sample_bgp_redist_profile_dict):
    """Return a sample BgpRedistributionProfileResponseModel."""
    return BgpRedistributionProfileResponseModel(**sample_bgp_redist_profile_dict)


@pytest.mark.usefixtures("load_env")
class TestBgpRedistributionProfileBase:
    """Base class for BgpRedistributionProfile tests."""
    ...
    def setup_method(self, mock_scm):
        ...
        self.client = BgpRedistributionProfile(self.mock_scm, max_limit=5000)


class TestBgpRedistributionProfile(TestBgpRedistributionProfileBase):
    """Test suite for BgpRedistributionProfile class."""

    def test_init(self):
        """Test initialization of BgpRedistributionProfile class."""
        client = BgpRedistributionProfile(self.mock_scm)
        assert client.api_client == self.mock_scm
        assert client.ENDPOINT == "/config/network/v1/bgp-redistribution-profiles"
    ...
    def test_init_with_custom_max_limit(self):
        ...
        client = BgpRedistributionProfile(self.mock_scm, max_limit=1000)
    ...
    def test_init_with_invalid_max_limit(self):
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            BgpRedistributionProfile(self.mock_scm, max_limit="invalid")
    ...
    def test_init_with_negative_max_limit(self):
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            BgpRedistributionProfile(self.mock_scm, max_limit=-1)
    ...
    def test_create(self, sample_bgp_redist_profile_dict):
        """Test create method."""
        self.mock_scm.post.return_value = sample_bgp_redist_profile_dict

        # Create a copy without the ID for create operation
        create_data = sample_bgp_redist_profile_dict.copy()
        create_data.pop("id")
        ...
        BgpRedistributionProfileCreateModel(**payload)

        # Check result
        assert isinstance(result, BgpRedistributionProfileResponseModel)
        assert result.name == sample_bgp_redist_profile_dict["name"]
        assert result.folder == sample_bgp_redist_profile_dict["folder"]

    def test_get(self, sample_bgp_redist_profile_dict):
        """Test get method."""
        self.mock_scm.get.return_value = sample_bgp_redist_profile_dict
        object_id = sample_bgp_redist_profile_dict["id"]
    ...
    def test_update(self, sample_bgp_redist_profile_dict):
        """Test update method."""
        self.mock_scm.put.return_value = sample_bgp_redist_profile_dict
        object_id = sample_bgp_redist_profile_dict["id"]

        update_model = BgpRedistributionProfileUpdateModel(**sample_bgp_redist_profile_dict)
    ...
    def test_list(self, sample_bgp_redist_profile_dict):
        """Test list method."""
        self.mock_scm.get.return_value = {
            "data": [sample_bgp_redist_profile_dict],
            "limit": 20,
            "offset": 0,
            "total": 1,
        }
        ...
        assert len(result) == 1
        assert isinstance(result[0], BgpRedistributionProfileResponseModel)
        assert result[0].name == sample_bgp_redist_profile_dict["name"]
    ...
    def test_list_pagination(self, sample_bgp_redist_profile_dict):
        """Test list method pagination."""
        profile1 = sample_bgp_redist_profile_dict.copy()
        profile1["id"] = str(uuid.uuid4())
        profile1["name"] = "profile1"

        profile2 = sample_bgp_redist_profile_dict.copy()
    ...
    def test_list_with_exclusions(self, sample_bgp_redist_profile_dict):
    ...
    def test_fetch(self, sample_bgp_redist_profile_dict):
        """Test fetch method."""
        self.mock_scm.get.return_value = sample_bgp_redist_profile_dict

        result = self.client.fetch(name="test-bgp-redist", folder="Test Folder")

        self.mock_scm.get.assert_called_once()
        call_args = self.mock_scm.get.call_args
        assert call_args[0][0] == self.client.ENDPOINT
        assert call_args[1]["params"]["name"] == "test-bgp-redist"
        assert call_args[1]["params"]["folder"] == "Test Folder"
    ...
    def test_fetch_with_invalid_response(self):
        ...
        self.mock_scm.get.return_value = {"name": "test-bgp-redist"}

        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist", folder="Test Folder")
    ...
    def test_fetch_response_errors(self):
        ...
        self.mock_scm.get.return_value = {"data": [{"id": valid_uuid, "name": "test-bgp-redist"}]}
        result = self.client.fetch(name="test-bgp-redist", folder="Test Folder")
        assert isinstance(result, BgpRedistributionProfileResponseModel)
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist", folder="Test Folder")
        assert "No matching BGP redistribution profile found" in str(excinfo.value)

        self.mock_scm.get.return_value = {
            "data": [{"name": "test-bgp-redist", "folder": "Test Folder"}]
        }
        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist", folder="Test Folder")
        assert "Response data item missing 'id' field" in str(excinfo.value)

    def test_fetch_with_original_response_format(self, sample_bgp_redist_profile_dict):
        """Test fetch method with original response format (direct object with id field)."""
        self.mock_scm.get.return_value = sample_bgp_redist_profile_dict

        result = self.client.fetch(
            name=sample_bgp_redist_profile_dict["name"],
            folder=sample_bgp_redist_profile_dict["folder"],
        )

        assert isinstance(result, BgpRedistributionProfileResponseModel)
        assert result.id == uuid.UUID(sample_bgp_redist_profile_dict["id"])
        assert result.name == sample_bgp_redist_profile_dict["name"]

    def test_fetch_with_list_response_format(self, sample_bgp_redist_profile_dict):
        """Test fetch method with list response format (data array with objects)."""
        profile_data = sample_bgp_redist_profile_dict.copy()
    ...
    def test_fetch_with_multiple_objects_in_data(self, sample_bgp_redist_profile_dict, monkeypatch):
        ...
        assert "Multiple BGP redistribution profiles found" in call_args

    def test_fetch_with_raw_list_response(self, sample_bgp_redist_profile_dict):
    ...
    def test_fetch_with_raw_list_response_multiple(
        self, sample_bgp_redist_profile_dict, monkeypatch
    ):
    ...
    def test_list_with_raw_list_response(self, sample_bgp_redist_profile_dict):
        ...
        assert isinstance(result, list)
        assert len(result) == 2
        assert all(isinstance(r, BgpRedistributionProfileResponseModel) for r in result)

    def test_delete(self, sample_bgp_redist_profile_dict):
        """Test delete method."""
        object_id = sample_bgp_redist_profile_dict["id"]
