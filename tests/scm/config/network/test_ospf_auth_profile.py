...
class TestOspfAuthProfile(TestOspfAuthProfileBase):
    ...
    def test_create(self, sample_ospf_auth_profile_dict):
        """Test create method."""
        self.mock_scm.post.return_value = sample_ospf_auth_profile_dict

        # Create a copy without the ID for create operation
        create_data = sample_ospf_auth_profile_dict.copy()
        create_data.pop("id")
    ...
    def test_fetch_with_raw_list_response(self, sample_ospf_auth_profile_dict):
    ...
    def test_fetch_with_raw_list_response_multiple(
        self, sample_ospf_auth_profile_dict, monkeypatch
    ):
    ...
    def test_list_with_raw_list_response(self, sample_ospf_auth_profile_dict):
        ...
        assert isinstance(result, list)
        assert len(result) == 2
        assert all(isinstance(r, OspfAuthProfileResponseModel) for r in result)

    def test_delete(self, sample_ospf_auth_profile_dict):
        """Test delete method."""
        object_id = sample_ospf_auth_profile_dict["id"]
