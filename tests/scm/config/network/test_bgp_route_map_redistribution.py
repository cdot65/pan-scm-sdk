"""Unit tests for the BgpRouteMapRedistribution class."""
...
@pytest.fixture
def sample_bgp_route_map_redist_dict():
    """Return a sample BGP route map redistribution dictionary."""
    return {
        "id": str(uuid.uuid4()),
        "name": "test-bgp-redist-map",
        "folder": "Test Folder",
        "bgp": {
            "ospf": {
                "route_map": [
                    {
                        "name": 10,
                        "action": "permit",
                        "match": {"as_path_access_list": "my-path"},
...
@pytest.fixture
def sample_bgp_route_map_redist_response(sample_bgp_route_map_redist_dict):
    """Return a sample BgpRouteMapRedistributionResponseModel."""
    return BgpRouteMapRedistributionResponseModel(**sample_bgp_route_map_redist_dict)


@pytest.mark.usefixtures("load_env")
class TestBgpRouteMapRedistributionBase:
    """Base class for BgpRouteMapRedistribution tests."""
    ...
    def setup_method(self, mock_scm):
        ...
        self.client = BgpRouteMapRedistribution(self.mock_scm, max_limit=5000)


class TestBgpRouteMapRedistribution(TestBgpRouteMapRedistributionBase):
    """Test suite for BgpRouteMapRedistribution class."""

    def test_init(self):
        """Test initialization of BgpRouteMapRedistribution class."""
        client = BgpRouteMapRedistribution(self.mock_scm)
        assert client.api_client == self.mock_scm
        assert client.ENDPOINT == "/config/network/v1/bgp-route-map-redistributions"
    ...
    def test_init_with_custom_max_limit(self):
        ...
        client = BgpRouteMapRedistribution(self.mock_scm, max_limit=1000)
    ...
    def test_init_with_invalid_max_limit(self):
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            BgpRouteMapRedistribution(self.mock_scm, max_limit="invalid")
    ...
    def test_init_with_negative_max_limit(self):
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            BgpRouteMapRedistribution(self.mock_scm, max_limit=-1)
    ...
    def test_create(self, sample_bgp_route_map_redist_dict):
        """Test create method."""
        self.mock_scm.post.return_value = sample_bgp_route_map_redist_dict

        # Create a copy without the ID for create operation
        create_data = sample_bgp_route_map_redist_dict.copy()
        create_data.pop("id")
        ...
        BgpRouteMapRedistributionCreateModel(**payload)

        # Check result
        assert isinstance(result, BgpRouteMapRedistributionResponseModel)
        assert result.name == sample_bgp_route_map_redist_dict["name"]
        assert result.folder == sample_bgp_route_map_redist_dict["folder"]

    def test_get(self, sample_bgp_route_map_redist_dict):
        """Test get method."""
        self.mock_scm.get.return_value = sample_bgp_route_map_redist_dict
        object_id = sample_bgp_route_map_redist_dict["id"]
    ...
    def test_update(self, sample_bgp_route_map_redist_dict):
        """Test update method."""
        self.mock_scm.put.return_value = sample_bgp_route_map_redist_dict
        object_id = sample_bgp_route_map_redist_dict["id"]

        update_model = BgpRouteMapRedistributionUpdateModel(**sample_bgp_route_map_redist_dict)
    ...
    def test_list(self, sample_bgp_route_map_redist_dict):
        """Test list method."""
        self.mock_scm.get.return_value = {
            "data": [sample_bgp_route_map_redist_dict],
            "limit": 20,
            "offset": 0,
            "total": 1,
        }
        ...
        assert len(result) == 1
        assert isinstance(result[0], BgpRouteMapRedistributionResponseModel)
        assert result[0].name == sample_bgp_route_map_redist_dict["name"]
    ...
    def test_list_pagination(self, sample_bgp_route_map_redist_dict):
        """Test list method pagination."""
        profile1 = sample_bgp_route_map_redist_dict.copy()
        profile1["id"] = str(uuid.uuid4())
        profile1["name"] = "profile1"

        profile2 = sample_bgp_route_map_redist_dict.copy()
    ...
    def test_list_with_exclusions(self, sample_bgp_route_map_redist_dict):
    ...
    def test_fetch(self, sample_bgp_route_map_redist_dict):
        """Test fetch method."""
        self.mock_scm.get.return_value = sample_bgp_route_map_redist_dict

        result = self.client.fetch(name="test-bgp-redist-map", folder="Test Folder")

        self.mock_scm.get.assert_called_once()
        call_args = self.mock_scm.get.call_args
        assert call_args[0][0] == self.client.ENDPOINT
        assert call_args[1]["params"]["name"] == "test-bgp-redist-map"
        assert call_args[1]["params"]["folder"] == "Test Folder"
    ...
    def test_fetch_with_invalid_response(self):
        ...
        self.mock_scm.get.return_value = {"name": "test-bgp-redist-map"}

        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist-map", folder="Test Folder")
    ...
    def test_fetch_response_errors(self):
        ...
        self.mock_scm.get.return_value = {
            "data": [{"id": valid_uuid, "name": "test-bgp-redist-map"}]
        }
        result = self.client.fetch(name="test-bgp-redist-map", folder="Test Folder")
        assert isinstance(result, BgpRouteMapRedistributionResponseModel)
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist-map", folder="Test Folder")
        assert "No matching BGP route map redistribution found" in str(excinfo.value)

        self.mock_scm.get.return_value = {
            "data": [{"name": "test-bgp-redist-map", "folder": "Test Folder"}]
        }
        with pytest.raises(InvalidObjectError) as excinfo:
            self.client.fetch(name="test-bgp-redist-map", folder="Test Folder")
        assert "Response data item missing 'id' field" in str(excinfo.value)

    def test_fetch_with_original_response_format(self, sample_bgp_route_map_redist_dict):
        """Test fetch method with original response format (direct object with id field)."""
        self.mock_scm.get.return_value = sample_bgp_route_map_redist_dict

        result = self.client.fetch(
            name=sample_bgp_route_map_redist_dict["name"],
            folder=sample_bgp_route_map_redist_dict["folder"],
        )

        assert isinstance(result, BgpRouteMapRedistributionResponseModel)
        assert result.id == uuid.UUID(sample_bgp_route_map_redist_dict["id"])
        assert result.name == sample_bgp_route_map_redist_dict["name"]

    def test_fetch_with_list_response_format(self, sample_bgp_route_map_redist_dict):
        """Test fetch method with list response format (data array with objects)."""
        profile_data = sample_bgp_route_map_redist_dict.copy()
    ...
    def test_fetch_with_multiple_objects_in_data(
        self, sample_bgp_route_map_redist_dict, monkeypatch
    ):
        ...
        assert "Multiple BGP route map redistributions found" in call_args

    def test_fetch_with_raw_list_response(self, sample_bgp_route_map_redist_dict):
    ...
    def test_list_with_raw_list_response(self, sample_bgp_route_map_redist_dict):
        ...
        assert isinstance(result, list)
        assert len(result) == 2
        assert all(isinstance(r, BgpRouteMapRedistributionResponseModel) for r in result)

    def test_delete(self, sample_bgp_route_map_redist_dict):
        """Test delete method."""
        object_id = sample_bgp_route_map_redist_dict["id"]
