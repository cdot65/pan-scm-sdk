"""Test module for Forwarding Profile User Locations configuration service.

This module contains unit tests for the User Locations configuration service.
"""
# tests/scm/config/mobile_agent/test_forwarding_profile_user_locations.py

from unittest.mock import MagicMock, patch

import pytest

from scm.config.mobile_agent import ForwardingProfileUserLocations
...
def sample_data():
    """Return a valid create payload for a user location."""
    return {
        "name": "branch-offices",
...
def sample_response(name=None):
    """Return a valid API response for a user location."""
    data = sample_data()
    if name is not None:
        data["name"] = name
    data["id"] = TEST_ID
...
class TestForwardingProfileUserLocations:
    """Test cases for the ForwardingProfileUserLocations service."""
    ...
    def service(self, mock_api_client):
        """Test fixture for ForwardingProfileUserLocations instance."""
        with patch("scm.config.isinstance", return_value=True):
            svc = ForwardingProfileUserLocations(mock_api_client)
    ...
    def test_init_with_default_max_limit(self, mock_api_client):
        ...
        with patch("scm.config.isinstance", return_value=True):
            svc = ForwardingProfileUserLocations(mock_api_client)
            assert svc._max_limit == ForwardingProfileUserLocations.DEFAULT_MAX_LIMIT

    def test_init_with_custom_max_limit(self, mock_api_client):
        """Test initialization with custom max limit."""
        with patch("scm.config.isinstance", return_value=True):
            svc = ForwardingProfileUserLocations(mock_api_client, max_limit=1000)
    ...
    def test_validate_max_limit_with_too_large_value(self, service):
        ...
        with pytest.raises(InvalidObjectError) as excinfo:
            service._validate_max_limit(ForwardingProfileUserLocations.ABSOLUTE_MAX_LIMIT + 1)
    ...
    def test_create(self, service, mock_api_client):
        ...
        assert isinstance(result, ForwardingProfileUserLocationResponseModel)
    ...
    def test_update(self, service, mock_api_client):
        ...
        update_model = ForwardingProfileUserLocationUpdateModel(id=TEST_ID, **sample_data())
    ...
    def test_list_with_dict_response(self, service, mock_api_client):
        ...
        assert len(result) == 2
        assert all(isinstance(item, ForwardingProfileUserLocationResponseModel) for item in result)
