# tests/scm/config/security/test_decryption_rule.py

"""Tests for decryption rule configuration."""
...
class TestDecryptionRuleBase:
    """Base class for Decryption Rule tests."""
    ...
    def setup_method(self, mock_scm):
        ...
        self.client = DecryptionRule(self.mock_scm, max_limit=5000)  # noqa


# -------------------- Test Classes Grouped by Functionality --------------------


class TestDecryptionRuleMaxLimit(TestDecryptionRuleBase):
    """Tests for max_limit functionality."""

    def test_default_max_limit(self):
        """Test that default max_limit is set correctly."""
        client = DecryptionRule(self.mock_scm)  # noqa
        assert client.max_limit == DecryptionRule.DEFAULT_MAX_LIMIT
        assert client.max_limit == 2500

    def test_custom_max_limit(self):
        """Test setting a custom max_limit."""
        client = DecryptionRule(self.mock_scm, max_limit=1000)  # noqa
    ...
    def test_invalid_max_limit_type(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            DecryptionRule(self.mock_scm, max_limit="invalid")  # noqa
    ...
    def test_max_limit_too_low(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            DecryptionRule(self.mock_scm, max_limit=0)  # noqa
    ...
    def test_max_limit_too_high(self):
        ...
        with pytest.raises(InvalidObjectError) as exc_info:
            DecryptionRule(self.mock_scm, max_limit=6000)  # noqa
        assert "max_limit exceeds maximum allowed value" in str(exc_info.value)


class TestDecryptionRuleList(TestDecryptionRuleBase):
    """Tests for listing Decryption Rule objects."""
    ...
    def test_list_valid(self):
        ...
        assert isinstance(existing_objects[0], DecryptionRuleResponseModel)
    ...
    def test_list_filters_valid(self):
        ...
        assert filtered_objects[0].action == DecryptionRuleAction.no_decrypt
    ...
    def test_list_filters_lists_empty(self):
        ...
        mock_response = {
            "data": [
                DecryptionRuleResponseFactory(
                    name="rule1", folder="Texas", action=DecryptionRuleAction.no_decrypt
                ).model_dump(by_alias=True)
            ]
        }
        self.mock_scm.get.return_value = mock_response  # noqa
    ...
    def test_list_rulebase(self, rulebase):
        ...
        mock_response = {
            "data": [
                DecryptionRuleResponseFactory().model_dump(by_alias=True),
            ],
            "offset": 0,
            "total": 1,
            "limit": 200,
    ...
    def test_list_pagination_multiple_pages(self):
        ...
        client = DecryptionRule(self.mock_scm, max_limit=2500)  # noqa

        # Create test data for three pages
        first_page = [
            DecryptionRuleResponseFactory(
                name=f"no-decrypt-rule-page1-{i}",
                folder="Texas",
                action=DecryptionRuleAction.no_decrypt,
                source=["10.0.0.0/24"],
                destination=["any"],
        ...
        third_page = [
            DecryptionRuleResponseFactory(
                name=f"no-decrypt-rule-page3-{i}",
                folder="Texas",
                action=DecryptionRuleAction.no_decrypt,
        ...
        assert isinstance(results[0], DecryptionRuleResponseModel)
        assert all(isinstance(obj, DecryptionRuleResponseModel) for obj in results)
        ...
        assert results[0].name == "no-decrypt-rule-page1-0"
        assert results[0].action == DecryptionRuleAction.no_decrypt
        assert results[0].source == ["10.0.0.0/24"]
        assert results[0].from_ == ["trust"]

        assert results[2500].name == "decrypt-rule-page2-0"
        assert results[2500].action == DecryptionRuleAction.decrypt
        assert results[2500].destination == ["10.1.0.0/24"]
        assert results[2500].to_ == ["trust"]

        assert results[5000].name == "no-decrypt-rule-page3-0"
        assert results[5000].action == DecryptionRuleAction.no_decrypt
        assert results[5000].source == ["192.168.1.0/24"]
        assert results[5000].from_ == ["dmz"]

    # -------------------- Filter-specific tests for full coverage --------------------
    ...
    def test_list_filter_by_action(self):
        ...
        filtered = self.client.list(folder="Texas", action=[DecryptionRuleAction.decrypt])
        assert len(filtered) == 1
        assert filtered[0].action == DecryptionRuleAction.decrypt
    ...
    def test_list_filter_by_tag_empty_tag_list(self):
        """Test that rules with empty tag lists are excluded from tag filter."""
        mock_response = {
            "data": [
                DecryptionRuleResponseFactory(
                    name="rule1",
        ...
        filtered = self.client.list(folder="Texas", tag=["some-tag"])
        assert len(filtered) == 0

    def test_list_invalid_category_filter(self):
        """Test that list method raises InvalidObjectError when 'category' filter is not a list."""
        mock_response = {
            "data": [
                DecryptionRuleResponseFactory(name="rule1", folder="Texas").model_dump(
                    by_alias=True
                )
            ]
        }
        ...
        with pytest.raises(InvalidObjectError):
            self.client.list(folder="Texas", category="social-networking")

    def test_list_invalid_service_filter(self):
        """Test that list method raises InvalidObjectError when 'service' filter is not a list."""
        mock_response = {
            "data": [
                DecryptionRuleResponseFactory(name="rule1", folder="Texas").model_dump(
                    by_alias=True
        ...
        with pytest.raises(InvalidObjectError):
            self.client.list(folder="Texas", service="tcp-443")
...
class TestDecryptionRuleCreate(TestDecryptionRuleBase):
    """Tests for creating Decryption Rule objects."""

    def test_create_valid_object(self):
        """Test creating an object with valid data."""
        test_object = DecryptionRuleCreateApiFactory.build()
        mock_response = DecryptionRuleResponseFactory.from_request(test_object)
        ...
        assert isinstance(created_object, DecryptionRuleResponseModel)
...
class TestDecryptionRuleGet(TestDecryptionRuleBase):
    """Tests for retrieving a specific Decryption Rule object."""

    def test_get_valid_object(self):
        """Test retrieving a specific object."""
        mock_response = DecryptionRuleResponseFactory.build()
        ...
        self.mock_scm.get.assert_called_once_with(  # noqa
            f"/config/security/v1/decryption-rules/{mock_response.id}"
        )
        assert isinstance(retrieved_object, DecryptionRuleResponseModel)
...
class TestDecryptionRuleUpdate(TestDecryptionRuleBase):
    """Tests for updating Decryption Rule objects."""

    def test_update_valid_object(self):
        """Test updating an object with valid data."""
        update_data = DecryptionRuleUpdateApiFactory.with_action_update(
        ...
        mock_response = DecryptionRuleResponseFactory.from_request(update_data)
        ...
        # Check endpoint
        assert call_args[0][0] == f"/config/security/v1/decryption-rules/{update_data.id}"

        # Check important payload fields
        payload = call_args[1]["json"]
        assert payload["name"] == "updated-rule"

        assert isinstance(updated_object, DecryptionRuleResponseModel)
        assert updated_object.id == mock_response.id
        assert updated_object.name == mock_response.name
        assert updated_object.action == DecryptionRuleAction.decrypt
...
class TestDecryptionRuleDelete(TestDecryptionRuleBase):
    """Tests for deleting Decryption Rule objects."""
    ...
    def test_delete_success(self):
        ...
        self.mock_scm.delete.assert_called_once_with(  # noqa
            f"/config/security/v1/decryption-rules/{object_id}",
            params={"position": "pre"},
        )
...
class TestDecryptionRuleFetch(TestDecryptionRuleBase):
    """Tests for fetching Decryption Rule objects by name."""

    def test_fetch_valid_object(self):
        """Test retrieving an object by its name using the `fetch` method."""
        mock_response_model = DecryptionRuleResponseFactory.build()
        ...
        assert isinstance(fetched_object, DecryptionRuleResponseModel)
...
class TestDecryptionRuleMove(TestDecryptionRuleBase):
    """Tests for moving Decryption Rule objects."""
    ...
    def test_move_valid(self):
        ...
        self.mock_scm.post.assert_called_once_with(  # noqa
            f"/config/security/v1/decryption-rules/{source_rule}:move",
            json=move_data,
        )
    ...
    def test_move_before_rule(self):
        ...
        move_config = DecryptionRuleMoveApiFactory.before_rule(
    ...
    def test_move_after_rule(self):
        ...
        source_rule = "123e4567-e89b-12d3-a456-426655440000"
        dest_rule_id = "987fcdeb-54ba-3210-9876-fedcba098765"
        move_config = DecryptionRuleMoveApiFactory.after_rule(
            dest_rule=dest_rule_id,
            rulebase=DecryptionRuleRulebase.PRE,
        )
    ...
    def test_move_invalid_destination_error(self):
        ...
        assert (
            "1 validation error for DecryptionRuleMoveModel\ndestination\n  Input should be 'top', 'bottom', 'before' or 'after'"
...
class TestDecryptionRuleModelMisc(TestDecryptionRuleBase):
    """Tests for miscellaneous DecryptionRule model behavior."""
    ...
    def test_decryption_rule_create_no_container(self):
        """Test that not providing any container raises a ValueError."""
        with pytest.raises(
            ValueError,
            match="Exactly one of 'folder', 'snippet', or 'device' must be provided.",
        ):
            DecryptionRuleCreateModel(name="test-rule", action="no-decrypt")

    def test_decryption_rule_create_multiple_containers(self):
