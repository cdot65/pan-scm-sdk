# tests/scm/config/security/test_url_access_profile.py

"""Tests for URL access profile service."""
...
class TestURLAccessProfileBase:
    """Base class for URL Access Profile tests."""
    ...
    def setup_method(self, mock_scm):
        ...
        self.client = URLAccessProfile(self.mock_scm)


# -------------------- Test Max Limit --------------------


class TestURLAccessProfileMaxLimit(TestURLAccessProfileBase):
    """Tests for max_limit validation in URLAccessProfile."""

    def test_default_max_limit(self):
        """Test that default max_limit is set correctly."""
        client = URLAccessProfile(self.mock_scm)
        assert client.max_limit == URLAccessProfile.DEFAULT_MAX_LIMIT

    def test_custom_max_limit(self):
        """Test that a custom max_limit is set correctly."""
        client = URLAccessProfile(self.mock_scm, max_limit=1000)
    ...
    def test_max_limit_too_low(self):
        ...
        with pytest.raises(InvalidObjectError):
            URLAccessProfile(self.mock_scm, max_limit=0)

    def test_max_limit_too_high(self):
        """Test validation when max_limit exceeds absolute maximum."""
        with pytest.raises(InvalidObjectError):
            URLAccessProfile(self.mock_scm, max_limit=10000)

    def test_max_limit_invalid_type(self):
        """Test validation when max_limit is not an integer."""
        with pytest.raises(InvalidObjectError):
            URLAccessProfile(self.mock_scm, max_limit="invalid")


# -------------------- Test List --------------------


class TestURLAccessProfileList(TestURLAccessProfileBase):
    """Tests for listing URL Access Profile objects."""
    ...
    def test_list_valid(self):
        ...
        assert isinstance(existing_objects[0], URLAccessProfileResponseModel)
    ...
    def test_list_pagination_multiple_pages(self):
        ...
        client = URLAccessProfile(self.mock_scm, max_limit=2500)  # noqa
        ...
        assert isinstance(results[0], URLAccessProfileResponseModel)
        assert all(isinstance(obj, URLAccessProfileResponseModel) for obj in results)

        assert self.mock_scm.get.call_count == 4  # noqa

        self.mock_scm.get.assert_any_call(  # noqa
            "/config/security/v1/url-access-profiles",
            params={
                "folder": "Texas",
                "limit": 2500,
                "offset": 0,
...
class TestURLAccessProfileCreate(TestURLAccessProfileBase):
    """Tests for creating URL Access Profile objects."""

    def test_create_valid_object(self):
        """Test creating an object with valid data."""
        test_object = URLAccessProfileCreateApiFactory.build()
        mock_response = URLAccessProfileResponseFactory.from_request(test_object)
        ...
        assert isinstance(created_object, URLAccessProfileResponseModel)
...
class TestURLAccessProfileGet(TestURLAccessProfileBase):
    """Tests for retrieving a specific URL Access Profile object."""

    def test_get_valid_object(self):
        """Test retrieving a specific object."""
        mock_response = URLAccessProfileResponseFactory.build()
        ...
        self.mock_scm.get.assert_called_once_with(  # noqa
            f"/config/security/v1/url-access-profiles/{mock_response.id}"
        )
        assert isinstance(retrieved_object, URLAccessProfileResponseModel)
...
class TestURLAccessProfileUpdate(TestURLAccessProfileBase):
    """Tests for updating URL Access Profile objects."""

    def test_update_valid_object(self):
        """Test updating an object with valid data."""
        update_data = URLAccessProfileUpdateApiFactory(
        ...
        mock_response = URLAccessProfileResponseFactory.from_request(update_data)
        ...
        assert call_args[0][0] == f"/config/security/v1/url-access-profiles/{update_data.id}"

        payload = call_args[1]["json"]
        assert payload["name"] == "updated-profile"

        assert isinstance(updated_object, URLAccessProfileResponseModel)
...
class TestURLAccessProfileDelete(TestURLAccessProfileBase):
    """Tests for deleting URL Access Profile objects."""
    ...
    def test_delete_success(self):
        ...
        self.mock_scm.delete.assert_called_once_with(  # noqa
            f"/config/security/v1/url-access-profiles/{object_id}"
        )
...
class TestURLAccessProfileFetch(TestURLAccessProfileBase):
    """Tests for fetching URL Access Profile objects by name."""

    def test_fetch_valid_object(self):
        """Test retrieving an object by its name using the `fetch` method."""
        mock_response_model = URLAccessProfileResponseFactory.build()
        ...
        assert isinstance(fetched_object, URLAccessProfileResponseModel)
