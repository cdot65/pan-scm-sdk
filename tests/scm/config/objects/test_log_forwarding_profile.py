...
class TestLogForwardingProfileListAndFetch:
    ...
    def test_list_with_filtering(self):
        ...
        log_forwarding_profile = LogForwardingProfile(api_client)
        profiles = log_forwarding_profile.list(
            folder="Shared", 
            log_type="traffic"
        )
    ...
    def test_apply_filters_directly(self):
        ...
        profile1 = LogForwardingProfileResponseModel(**create_sample_log_forwarding_profile_response())
        profile1.match_list[0].log_type = "traffic"
        
        profile2 = LogForwardingProfileResponseModel(**create_sample_log_forwarding_profile_response())
        profile2.match_list[0].log_type = "threat"
        
        # Create profile3 with url log type
        profile3 = LogForwardingProfileResponseModel(**create_sample_log_forwarding_profile_response())
        profile3.match_list[0].log_type = "url"
        ...
        filtered = log_forwarding_profile._apply_filters(
            [test_profile1, test_profile2, test_profile3],
            {"tags": ["test-tag"]}
        )
    ...
    def test_fetch(self):
        ...
        log_forwarding_profile = LogForwardingProfile(api_client)
        profile = log_forwarding_profile.fetch(
            name="test-log-profile", 
            folder="Shared"
        )
