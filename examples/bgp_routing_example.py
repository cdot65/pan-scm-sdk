...
def main():
    ...
    if not client_id or not client_secret or not tsg_id:
        logger.error("Missing required environment variables. Please set SCM_CLIENT_ID, SCM_CLIENT_SECRET, and SCM_TSG_ID.")
        sys.exit(1)
    ...
    try:
        ...
        logger.info(f"Accept route over Service Connection: {current_settings.accept_route_over_SC}")
        
        if current_settings.outbound_routes_for_services:
            logger.info(f"Outbound routes for services: {', '.join(current_settings.outbound_routes_for_services)}")
        
        if current_settings.routing_preference:
            preference_type = "Default" if hasattr(current_settings.routing_preference, "default") else "Hot Potato"
    ...
    try:
        ...
        if confirm.lower() == 'y':
            ...
            logger.info(f"New routing preference: Hot Potato")
    ...
    try:
        ...
        if confirm.lower() == 'y':
            ...
            logger.info("\nBGP routing settings updated successfully:")
            logger.info(f"New backbone routing: {updated_settings.backbone_routing}")
            logger.info(f"New routing preference: Default")
