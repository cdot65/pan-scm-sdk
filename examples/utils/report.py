...
class ReportGenerator:
    ...
    def generate_csv(self, 
                    headers: List[str], 
                    data: List[List[Any]], 
                    summary_data: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            self.logger.error(f"Failed to write CSV report", e)
    ...
    def generate_pdf(self, 
        ...
        try:
            ...
            if len(data) > row_limit:
                ...
                elements.append(Paragraph(truncate_notice, ParagraphStyle(
                    "Notice", 
                    parent=normal_style, 
            ...
            table.setStyle(TableStyle([
                ...
                ('BACKGROUND', (0, 2), (-1, -1), colors.lightgrey),  # Start with row 2 (0-indexed)
                ('BACKGROUND', (0, 1), (-1, -1), colors.white),
            ...
            try:
                ...
                self.logger.success(f"Simplified PDF report generated at fallback location: {fallback_path}")
                return str(fallback_path)
                
            except Exception as fallback_error:
                self.logger.error(f"Failed to write fallback PDF", fallback_error)
                return None
    
    def _calculate_column_widths(self, 
                               headers: List[str], 
                               data: List[List[Any]], 
                               pagesize: Tuple[float, float]) -> List[float]:
        """
        ...
        for i in range(len(headers)):
            ...
            data_lengths = [len(str(row[i])) if i < len(row) else 0 for row in data[:100]]  # Sample first 100 rows
