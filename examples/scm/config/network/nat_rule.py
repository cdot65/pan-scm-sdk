...
def create_source_nat_rule(nat_rules, folder="Texas"):
    ...
    log_info(f"  - Type: Source NAT with dynamic IP and port")
    log_info(f"  - Source: 10.0.0.0/24 (internal network)")
    log_info(f"  - Translated addresses: 192.168.1.100, 192.168.1.101 (public IPs)")
...
def create_source_nat_rule_with_interface(nat_rules, folder="Texas"):
    ...
    interface_addr = InterfaceAddress(interface="$eth-internet", ip="192.168.1.1")

    dynamic_ip_port = DynamicIpAndPort(
        type="dynamic_ip_and_port", interface_address=interface_addr
    )

    source_trans = SourceTranslation(dynamic_ip_and_port=dynamic_ip_port)
...
def create_static_nat_rule(nat_rules, folder="Texas"):
    ...
    static_ip = StaticIp(
        ...
        bi_directional="yes",  # Must be string "yes" or "no", not boolean
    )

    source_trans = SourceTranslation(static_ip=static_ip)
...
def create_destination_nat_rule(nat_rules, folder="Texas"):
    ...
    dest_trans = DestinationTranslation(
        translated_address="10.0.0.100", translated_port=80
    )
...
def create_dynamic_dest_nat_rule(nat_rules, folder="Texas"):
    ...
    dest_trans = DestinationTranslation(
        translated_address="10.0.0.200", translated_port=8080
    )
...
def create_bidirectional_nat_rule(nat_rules, folder="Texas"):
    ...
    static_ip = StaticIp(translated_address="192.168.10.100", bi_directional="yes")
...
def create_port_forwarding_rule(nat_rules, folder="Texas"):
    ...
    dest_trans = DestinationTranslation(
        translated_address="10.10.10.100", 
...
def create_nptv6_rule(nat_rules, folder="Texas"):
    ...
    static_ip = StaticIp(
        ...
        bi_directional="no",  # String "yes" or "no" is required, not a boolean
    )
...
def create_outbound_nat_different_interfaces(nat_rules, folder="Texas"):
    ...
    interface_addr = InterfaceAddress(
        interface="$eth-internet", ip="203.0.113.50"  # Primary internet connection
    )
...
def create_post_nat_rule(nat_rules, folder="Texas"):
    ...
    dynamic_ip_port = DynamicIpAndPort(
        type="dynamic_ip_and_port", translated_address=["192.168.5.100"]
    )
...
def create_bulk_nat_rules(nat_rules, folder="Texas"):
    ...
    for service_info in services:
        dest_trans = DestinationTranslation(
            translated_address="10.50.10.100", translated_port=service_info["port"]
        )
...
def create_nat_rules_for_multiple_sites(nat_rules, folder="Texas"):
    ...
    for site in branch_sites:
        ...
        dynamic_ip_port = DynamicIpAndPort(
            type="dynamic_ip_and_port", translated_address=[site["external_ip"]]
        )
    ...
    for idx, rule_id in enumerate(rule_ids):
        ...
        try:
            ...
            if rule.source_translation:
                if hasattr(rule.source_translation, "dynamic_ip_and_port") and rule.source_translation.dynamic_ip_and_port:
                    source_trans_type = "Dynamic IP and Port"
                    if hasattr(rule.source_translation.dynamic_ip_and_port, "translated_address") and rule.source_translation.dynamic_ip_and_port.translated_address:
                        translated_addresses = ", ".join(rule.source_translation.dynamic_ip_and_port.translated_address)
                        source_trans_type += f" ({translated_addresses})"
                    elif hasattr(rule.source_translation.dynamic_ip_and_port, "interface_address") and rule.source_translation.dynamic_ip_and_port.interface_address:
                        interface = rule.source_translation.dynamic_ip_and_port.interface_address.interface
                        source_trans_type += f" (Interface: {interface})"
                elif hasattr(rule.source_translation, "dynamic_ip") and rule.source_translation.dynamic_ip:
                    source_trans_type = "Dynamic IP"
                    if hasattr(rule.source_translation.dynamic_ip, "translated_address") and rule.source_translation.dynamic_ip.translated_address:
                        translated_addresses = ", ".join(rule.source_translation.dynamic_ip.translated_address)
                        source_trans_type += f" ({translated_addresses})"
                elif hasattr(rule.source_translation, "static_ip") and rule.source_translation.static_ip:
                    bi_dir = ""
                    if hasattr(rule.source_translation.static_ip, "bi_directional") and rule.source_translation.static_ip.bi_directional == "yes":
...
def parse_arguments():
    ...
    rule_group.add_argument(
        "--source-nat", 
        action="store_true",
        help="Create source NAT examples"
    )
    rule_group.add_argument(
        "--dest-nat", 
        action="store_true", 
        help="Create destination NAT examples"
    )
    rule_group.add_argument(
        "--special-nat", 
        action="store_true",
...
def main():
    ...
    create_all = args.all or not (args.source_nat or args.dest_nat or args.special_nat or args.bulk_nat)
    ...
    try:
        ...
        if create_all or args.source_nat:
            ...
            log_info("Creating common outbound NAT rule patterns used for internet access")
            ...
            source_nat_interface = create_source_nat_rule_with_interface(nat_rules, folder_name)
        ...
        if create_all or args.dest_nat:
            ...
            outbound_interface_nat = create_outbound_nat_different_interfaces(nat_rules, folder_name)
        ...
        if created_rules and not args.no_report:
            ...
            report_file = generate_nat_rule_report(nat_rules, created_rules, execution_time_so_far)
            if report_file:
                log_success(f"Generated NAT rules report: {report_file}")
                log_info(f"The report contains details of all {len(created_rules)} NAT rules created")
            else:
        ...
        log_section("CLEANUP")
        if skip_cleanup:
            log_info(f"SKIP_CLEANUP is set to true - preserving {len(created_rules)} NAT rules")
            log_info("To clean up these rules, run the script again with SKIP_CLEANUP unset or set to false")
        else:
