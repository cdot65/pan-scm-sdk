...
def create_technology_filter(app_filters, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Technology Filter")
...
def create_behavioral_filter(app_filters, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Behavioral Filter")
...
def create_saas_filter(app_filters, folder="Texas"):
    ...
    try:
        ...
        log_success(f"Created application filter: {new_filter.name}")
        log_info(f"  - Object ID: {new_filter.id}")
        log_operation_complete("SaaS application filter creation", f"Filter: {new_filter.name}")
        return new_filter
    except NameNotUniqueError as e:
        log_error("Filter name conflict", e.message)
        log_info("Try using a different filter name or check existing objects")
...
def create_comprehensive_filter(app_filters, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Comprehensive Filter")
...
def list_and_filter_app_filters(app_filters):
    ...
    try:
        ...
        logger.info(
            f"Found {len(category_filters)} application filters with 'category' in the name"
        )
    except Exception as e:
...
def create_bulk_app_filter_objects(app_filters, folder="Texas"):
    ...
    for filter_config in filter_configs:
        try:
            ...
            logger.info(f"Created application filter: {new_filter.name} with ID: {new_filter.id}")
...
def generate_app_filter_report(app_filters, filter_ids, execution_time):
    ...
    for idx, filter_id in enumerate(filter_ids):
        ...
        try:
            ...
            categories = (
                ", ".join(app_filter.category)
                if hasattr(app_filter, "category") and app_filter.category
                else "None"
            )
            sub_categories = (
                ", ".join(app_filter.sub_category)
                if hasattr(app_filter, "sub_category") and app_filter.sub_category
                else "None"
            )
            technologies = (
                ", ".join(app_filter.technology)
                if hasattr(app_filter, "technology") and app_filter.technology
                else "None"
            )
            risk_levels = (
                ", ".join(map(str, app_filter.risk))
                if hasattr(app_filter, "risk") and app_filter.risk
                else "None"
            )
...
def parse_arguments():
    ...
    object_group.add_argument(
        "--category", action="store_true", help="Create category-based filter examples"
    )
    object_group.add_argument(
        "--technology", action="store_true", help="Create technology-based filter examples"
    )
    object_group.add_argument(
        "--risk", action="store_true", help="Create risk-based filter examples"
    )
    object_group.add_argument(
        "--behavioral", action="store_true", help="Create behavioral filter examples"
    )
    object_group.add_argument("--saas", action="store_true", help="Create SaaS filter examples")
    object_group.add_argument(
        "--comprehensive", action="store_true", help="Create comprehensive filter examples"
    )
    object_group.add_argument("--bulk", action="store_true", help="Create bulk filter examples")
    object_group.add_argument(
        "--all", action="store_true", help="Create all application filter types (default behavior)"
    )
...
def main():
    ...
    try:
        ...
        if create_all or args.category:
            ...
            log_success("Created category-based application filter")
        ...
        if create_all or args.technology:
            ...
            log_success("Created technology-based application filter")
        ...
        if create_all or args.risk:
            ...
            log_success("Created risk-based application filter")
        ...
        if create_all or args.behavioral:
            ...
            log_success("Created behavior-based application filter")
        ...
        if create_all or args.saas:
            ...
            log_success("Created SaaS application filter")
        ...
        if create_all or args.comprehensive:
            ...
            log_success("Created comprehensive application filter")
        ...
        if created_filters:
            ...
            fetch_and_update_filter(app_filters, created_filters[0])
        ...
        list_and_filter_app_filters(app_filters)
        ...
        if created_filters and not args.no_report:
            ...
            report_file = generate_app_filter_report(
                app_filters, created_filters, execution_time_so_far
            )
            if report_file:
                log_success(f"Generated application filters report: {report_file}")
                log_info(
                    f"The report contains details of all {len(created_filters)} application filter objects created"
                )
            else:
        ...
        if skip_cleanup:
            log_info(
                f"SKIP_CLEANUP is set to true - preserving {len(created_filters)} application filter objects"
            )
            log_info(
            ...
            log_operation_start(
                f"Cleaning up {len(created_filters)} created application filter objects"
            )
