...
def create_basic_application_group(app_groups, folder="Texas"):
    ...
    log_info("  - Type: Basic Application Group")
    ...
    try:
        ...
        log_operation_complete("Basic application group creation", f"Group: {new_group.name}")
        return new_group
        ...
        log_error("Unexpected error creating application group", str(e))
...
def create_custom_application_group(app_groups, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Custom Application Group")
    ...
    try:
        ...
        log_info(f"  - Object ID: {new_group.id}")
        log_info(f"  - Members: {', '.join(new_group.members)}")
        log_operation_complete("Custom application group creation", f"Group: {new_group.name}")
        return new_group
    except NameNotUniqueError as e:
        log_error("Group name conflict", e.message)
        log_info("Try using a different group name or check existing objects")
...
def create_nested_application_group(app_groups, basic_group_id, custom_group_id, folder="Texas"):
    ...
    try:
        ...
        log_info("Configuration details:")
        log_info("  - Type: Nested Application Group")
        ...
        log_info(f"  - Object ID: {new_group.id}")
        log_info(f"  - Members: {', '.join(new_group.members)}")
        log_operation_complete("Nested application group creation", f"Group: {new_group.name}")
        return new_group
    except NotFoundError as e:
        log_error("Referenced group not found", e.message)
        ...
        log_error("Unexpected error creating nested application group", str(e))
...
def create_mixed_application_group(app_groups, basic_group_id, folder="Texas"):
    ...
    try:
        ...
        log_info("Configuration details:")
        log_info("  - Type: Mixed Application Group")
        ...
        log_info(f"  - Object ID: {new_group.id}")
        log_info(f"  - Members: {', '.join(new_group.members)}")
        log_operation_complete("Mixed application group creation", f"Group: {new_group.name}")
        return new_group
    except NotFoundError as e:
        log_error("Referenced group not found", e.message)
        ...
        log_error("Unexpected error creating mixed application group", str(e))
...
def fetch_and_update_application_group(app_groups, group_id):
    ...
    try:
        ...
        app_group.members = app_group.members + [
            "smtp",
            "pop3",
        ]  # Using apps known to work from logs
...
def cleanup_application_group_objects(app_groups, group_ids):
    ...
    for group_id in group_ids:
        try:
            ...
            groups_info[group_id] = {"name": group.name, "members": group.members}
        except Exception:
...
def create_bulk_application_group_objects(app_groups, folder="Texas"):
    ...
    for group_config in group_configs:
        try:
            ...
            logger.info(f"Created application group: {new_group.name} with ID: {new_group.id}")
...
def parse_arguments():
    ...
    object_group.add_argument(
        "--basic", action="store_true", help="Create basic application group examples"
    )
    object_group.add_argument(
        "--custom", action="store_true", help="Create custom application group examples"
    )
    object_group.add_argument(
        "--nested", action="store_true", help="Create nested application group examples"
    )
    object_group.add_argument(
        "--mixed", action="store_true", help="Create mixed application group examples"
    )
    object_group.add_argument(
        "--bulk", action="store_true", help="Create bulk application group examples"
    )
    object_group.add_argument(
        "--all", action="store_true", help="Create all application group types (default behavior)"
    )
...
def main():
    ...
    create_all = args.all or not (
        args.basic or args.custom or args.nested or args.mixed or args.bulk
    )
    ...
    try:
        ...
        if create_all or args.basic:
            ...
            log_success("Created basic application group")
        ...
        if create_all or args.custom:
            ...
            log_success("Created custom application group")
        ...
        if (create_all or args.nested) and len(created_groups) >= 2:
            ...
            log_success("Created nested application group")
        ...
        if (create_all or args.mixed) and created_groups:
            ...
            mixed_group = create_mixed_application_group(app_groups, created_groups[0], folder_name)
            if mixed_group:
                created_groups.append(mixed_group.id)
                object_count += 1

            log_success("Created mixed application group")
        ...
        if created_groups:
            ...
            fetch_and_update_application_group(app_groups, created_groups[0])
        ...
        list_and_filter_application_groups(app_groups)
        ...
        if created_groups and not args.no_report:
            ...
            report_file = generate_application_group_report(
                app_groups, created_groups, execution_time_so_far
            )
            if report_file:
                log_success(f"Generated application groups report: {report_file}")
                log_info(
                    f"The report contains details of all {len(created_groups)} application group objects created"
                )
            else:
        ...
        if skip_cleanup:
            log_info(
                f"SKIP_CLEANUP is set to true - preserving {len(created_groups)} application group objects"
            )
            log_info(
            ...
            log_operation_start(
                f"Cleaning up {len(created_groups)} created application group objects"
            )
