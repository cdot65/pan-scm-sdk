...
def create_ipv4_network_address(addresses, folder="Texas"):
    ...
    try:
        ...
        log_operation_complete("IPv4 network address creation", f"Address: {new_address.name}")
        return new_address
...
def create_ipv4_host_address(addresses, folder="Texas"):
    ...
    try:
        ...
        log_info(f"  - Object ID: {new_address.id}")
        log_info(f"  - Description: {new_address.description}")
        log_operation_complete("IPv4 host address creation", f"Address: {new_address.name}")
        return new_address
    except NameNotUniqueError as e:
        log_error("Address name conflict", e.message)
        log_info("Try using a different address name or check existing objects")
...
def create_ipv4_range_address(addresses, folder="Texas"):
    ...
    try:
        ...
        log_info(f"  - Object ID: {new_address.id}")
        log_info(f"  - Description: {new_address.description}")
        log_operation_complete("IPv4 range address creation", f"Address: {new_address.name}")
        return new_address
    except NameNotUniqueError as e:
        log_error("Address name conflict", e.message)
        log_info("Try using a different address name or check existing objects")
...
def create_ipv6_address(addresses, folder="Texas"):
    ...
    try:
        ...
        log_info(f"  - Object ID: {new_address.id}")
        log_info(f"  - Description: {new_address.description}")
        log_operation_complete("IPv6 network address creation", f"Address: {new_address.name}")
        return new_address
    except NameNotUniqueError as e:
        log_error("Address name conflict", e.message)
        log_info("Try using a different address name or check existing objects")
...
def create_fqdn_address(addresses, folder="Texas"):
    ...
    try:
        ...
        log_info(f"  - Object ID: {new_address.id}")
        log_info(f"  - Description: {new_address.description}")
        log_operation_complete("FQDN address creation", f"Address: {new_address.name}")
        return new_address
    except NameNotUniqueError as e:
        log_error("Address name conflict", e.message)
        log_info("Try using a different address name or check existing objects")
...
def create_bulk_address_objects(addresses, folder="Texas"):
    ...
    for address_config in address_configs:
        try:
            ...
            logger.info(f"Created address object: {new_address.name} with ID: {new_address.id}")
        ...
        try:
            ...
            address_data.append(
                [
                    ...
                    address.created_on.strftime("%Y-%m-%d %H:%M:%S")
                    if hasattr(address, "created_on") and address.created_on
                    else "Unknown",
...
def main():
    ...
    try:
        ...
        if create_all or args.ipv6:
            ...
            log_success("Created IPv6 address objects")
        ...
        if create_all or args.fqdn:
            ...
            log_success("Created FQDN address objects")
        ...
        list_and_filter_addresses(addresses)
        ...
        if created_addresses and not args.no_report:
            ...
            report_file = generate_address_report(
                addresses, created_addresses, execution_time_so_far
            )
            if report_file:
                log_success(f"Generated address objects report: {report_file}")
                log_info(
                    f"The report contains details of all {len(created_addresses)} address objects created"
                )
            else:
