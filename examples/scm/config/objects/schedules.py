...
def create_daily_schedule(
    schedule_client: schedules.Schedule, name: str, description: str, folder: str = "Texas"
):
    ...
    try:
        ...
        data = {
            ...
            "folder": folder,
            "schedule_type": {"recurring": {"daily": ["08:00-17:00"]}},
        }
        ...
        log_error("Failed to create daily schedule", str(e))
        raise
...
def create_weekly_schedule(
    ...
    try:
        ...
        log_error("Failed to create weekly schedule", str(e))
        raise


def create_non_recurring_schedule(
    schedule_client: schedules.Schedule, name: str, description: str, folder: str = "Texas"
):
    ...
    try:
        ...
        data = {
            ...
            "description": description,
            "folder": folder,
            "schedule_type": {"non_recurring": [non_recurring_range]},
        }
        ...
        log_error("Failed to create non-recurring schedule", str(e))
        raise
...
def update_schedule(schedule_client: schedules.Schedule, schedule_id: str, description: str):
    ...
    try:
        ...
        log_error("Failed to update schedule", str(e))
        raise
...
def list_schedules(schedule_client: schedules.Schedule, schedule_type=None, folder: str = "Texas"):
    ...
    """
    log_info(
        f"Listing schedules from folder '{folder}'"
        + (f" of type: {schedule_type}" if schedule_type else "")
    )
    ...
    try:
        ...
        if schedule_type:
            if schedule_type == "recurring":
                schedules_list = [
                    ...
                    if (
                        hasattr(s.schedule_type, "recurring")
                ...
                schedules_list = [
                    ...
                    for s in schedules_list
                    if (
                        hasattr(s.schedule_type, "non_recurring")
                        and s.schedule_type.non_recurring is not None
                    )
                ]
        ...
        log_error("Failed to list schedules", str(e))
        raise
...
def fetch_schedule_by_name(schedule_client: schedules.Schedule, name: str, folder: str = "Texas"):
    ...
    try:
        ...
        log_error("Failed to fetch schedule", str(e))
        raise
...
def delete_schedule(schedule_client: schedules.Schedule, schedule_id: str):
    ...
    try:
        ...
        log_error("Failed to delete schedule", str(e))
        raise
...
def generate_schedules_report(schedules_list, filename: str) -> None:
    ...
    try:
        with open(filename, mode="w", newline="") as file:
            ...
            for schedule in schedules_list:
                ...
                if hasattr(schedule, "schedule_type"):
                    if (
                        hasattr(schedule.schedule_type, "recurring")
                        and schedule.schedule_type.recurring
                    ):
                        if (
                            hasattr(schedule.schedule_type.recurring, "daily")
                            and schedule.schedule_type.recurring.daily
                        ):
                            schedule_type_info = "Recurring - Daily"
                        elif (
                            hasattr(schedule.schedule_type.recurring, "weekly")
                            and schedule.schedule_type.recurring.weekly
                        ):
                            schedule_type_info = "Recurring - Weekly"
                    elif (
                        hasattr(schedule.schedule_type, "non_recurring")
                        and schedule.schedule_type.non_recurring
                    ):
                ...
                row = {
                    ...
                    "created_on": str(schedule.created_on)
                    if hasattr(schedule, "created_on")
                    else "",
                    "created_by": schedule.created_by if hasattr(schedule, "created_by") else "",
                    "modified_on": str(schedule.modified_on)
                    if hasattr(schedule, "modified_on")
                    else "",
        ...
        log_error("Failed to generate report", str(e))
        raise
...
def parse_arguments():
    ...
    object_group.add_argument("--daily", action="store_true", help="Create daily schedule examples")
    object_group.add_argument(
        "--weekly", action="store_true", help="Create weekly schedule examples"
    )
    object_group.add_argument(
        "--non-recurring", action="store_true", help="Create non-recurring schedule examples"
    )
    object_group.add_argument(
        "--all", action="store_true", help="Create all schedule object types (default behavior)"
    )
...
def main():
    ...
    try:
        ...
        if create_all or args.daily:
            ...
            log_success("Created daily schedule objects")
        ...
        if create_all or args.weekly:
            ...
            log_success("Created weekly schedule objects")
        ...
        if create_all or args.non_recurring:
            ...
            log_success("Created non-recurring schedule objects")
        ...
        try:
            recurring_schedules = list_schedules(
                schedule_client, schedule_type="recurring", folder=folder_name
            )
        ...
        if created_schedules and not args.no_report:
            ...
            log_info("The report contains details of all schedules in the system")
        elif args.no_report:
        ...
        if skip_cleanup:
            log_info(
                f"SKIP_CLEANUP is set to true - preserving {len(created_schedules)} schedule objects"
            )
            log_info(
