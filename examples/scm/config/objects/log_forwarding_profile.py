...
def create_traffic_log_profile(profiles, folder="Texas"):
    ...
    log_info("  - Type: Traffic Log Profile")
    ...
    try:
        ...
        log_error("Unexpected error creating log forwarding profile", str(e))
...
def create_threat_log_profile(profiles, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Threat Log Profile")
...
def create_wildfire_log_profile(profiles, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: WildFire Log Profile")
...
def create_multi_type_profile(profiles, folder="Texas"):
    ...
    log_info("Configuration details:")
    log_info("  - Type: Multi-type Log Profile")
            ...
            "match_list": [
                {
                    ...
                    "send_syslog": [
                        "test123"
                    ],  # Using existing syslog profile from the environment
                }
            ],
        },
        {
...
def parse_arguments():
    ...
    profile_group.add_argument(
        "--traffic", action="store_true", help="Create traffic log profile examples"
    )
    profile_group.add_argument(
        "--threat", action="store_true", help="Create threat log profile examples"
    )
    profile_group.add_argument("--url", action="store_true", help="Create URL log profile examples")
    profile_group.add_argument(
        "--wildfire", action="store_true", help="Create WildFire log profile examples"
    )
    profile_group.add_argument(
        "--multi", action="store_true", help="Create multi-type log profile examples"
    )
    profile_group.add_argument(
        "--bulk", action="store_true", help="Create bulk log profile examples"
    )
    profile_group.add_argument(
        "--all", action="store_true", help="Create all log profile types (default behavior)"
    )
    ...
    # Folder
    parser.add_argument(
        "--folder", type=str, default="Texas", help="Folder name in SCM to create profiles in"
    )
            ...
            if profile.match_list:
                for match in profile.match_list:
                    if match.send_syslog and syslog_name in match.send_syslog:
                        log_success(
                            f"Found syslog server profile '{syslog_name}' in existing profiles"
                        )
                        return True
...
def main():
    ...
    create_all = args.all or not (
        args.traffic or args.threat or args.url or args.wildfire or args.multi or args.bulk
    )
    ...
    try:
        ...
        if not syslog_exists:
            log_warning(
                "The script may encounter errors if the required syslog server profile doesn't exist"
            )
            log_info(
                "You can modify the script to use an existing syslog server profile in your environment"
            )
        ...
        if create_all or args.threat:
            ...
            log_success("Created threat log forwarding profiles")
        ...
        if create_all or args.url:
            ...
            log_success("Created URL log forwarding profiles")
        ...
        if create_all or args.wildfire:
            ...
            log_success("Created WildFire log forwarding profiles")
        ...
        if create_all or args.multi:
            ...
            log_success("Created multi-type log forwarding profiles")
        ...
        if created_profiles:
            ...
            fetch_and_update_profile(profiles, created_profiles[0])
        ...
        list_and_filter_profiles(profiles)
        ...
        if created_profiles and not args.no_report:
            ...
            if report_file:
                ...
                log_info(
                    f"The report contains details of all {len(created_profiles)} profiles created"
                )
            else:
        ...
        if skip_cleanup:
            log_info(
                f"SKIP_CLEANUP is set to true - preserving {len(created_profiles)} log forwarding profiles"
            )
            log_info(
            ...
            log_operation_start(
                f"Cleaning up {len(created_profiles)} created log forwarding profiles"
            )
        ...
        log_info(f"Average time per profile: {execution_time / max(object_count, 1):.2f} seconds")
