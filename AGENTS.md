...
make docs           # Build the Docusaurus site (docs-site/)
...
│   └── mobile_agent/  # auth_settings.py, agent_versions.py, forwarding_profiles.py
...
1. **Service**: Copy `SDK_SERVICE_TEMPLATE.py` (repo root) → `scm/config/{category}/{resource}.py`
2. **Models**: Create `scm/models/{category}/{resource}.py`
3. **Client**: Register in `scm/client.py`
4. **Tests**: Add `tests/scm/config/{category}/test_{resource}.py` + model tests
5. **Docs**: Add `docs-site/docs/sdk/config/{category}/{resource}.md` and the matching `models/` page, then register both in `docs-site/sidebars.ts`
...
Docs are a Docusaurus site under `docs-site/` (served at `/pan-scm-sdk/`). Contributor and policy docs live there too — Contributing, Code of Conduct, Security, Support, and the SDK styling / Pydantic models guides are pages under `docs-site/docs/about/`, not root markdown files. `make docs` runs a production build; `onBrokenLinks: 'throw'` means broken internal links fail the build, so verify with `make docs` before pushing doc changes.
...
- See the [SDK Service Styling Guide](docs-site/docs/about/sdk-styling-guide.md) and [Pydantic Models Guide](docs-site/docs/about/pydantic-models-guide.md) for the full conventions.
